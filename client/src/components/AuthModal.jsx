...
export default function AuthModal({ 
  ...
  // OAuth token client ref
  const tokenClientRef = useRef(null);

  const completeGoogleLogin = async (selectedName, selectedEmail, picture) => {
    const finalName = selectedName && selectedName.trim() ? selectedName.trim() : 'Heet Chheda';
    const finalEmail = selectedEmail && selectedEmail.trim() ? selectedEmail.trim() : 'heet.chheda06@gmail.com';
      ...
      try {
        ...
        try {
          if (response && response.credential) {
            ...
            if (payload && payload.picture) picture = payload.picture;
          }
        } catch (e) {}
