    ...
    const handleGoogleResponse = async (response) => {
      try {
        ...
        const res = await fetch('/api/auth/google', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ credential: response.credential, name: googleName, email: googleEmail })
        });
        const data = await res.json();
        if (data.success && onGoogleLogin) {
          onGoogleLogin(data.user || { name: googleName, email: googleEmail }, data.token);
        } else if (onGoogleLogin) {
          onGoogleLogin({ name: googleName, email: googleEmail }, 'token_google_demo');
        }
      } catch (err) {
    ...
    script.onload = () => {
      if (window.google?.accounts?.id) {
        try {
          ...
          // Enable Google One-Tap prompt for 1-click login
      ...
      <div className="modal-card glass auth-card-wide">
        ...
        {isGooglePrePage ? (
          <div style={{ padding: '10px 4px' }}>
            <div style={{ textAlign: 'center', marginBottom: '22px' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '54px', height: '54px', borderRadius: '18px', background: '#F8FAFC', border: '1.5px solid #E2E8F0', marginBottom: '12px', boxShadow: '0 4px 14px rgba(0,0,0,0.06)' }}>
              ...
              <p style={{ color: '#64748B', fontSize: '13px', margin: '4px 0 0 0', fontWeight: 600 }}>
                Choose an account to continue to AURA Smart Dining
              </p>
            </div>
            ...
            {/* Custom Google Profile Card */}
            <div style={{ background: '#F8FAFC', padding: '18px', borderRadius: '18px', border: '1.5px solid #CBD5E1', marginBottom: '16px' }}>
              <span style={{ fontSize: '11px', fontWeight: 800, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '10px' }}>
                Or enter custom Google account name &amp; email:
              </span>
              <input
                type="text"
            ...
            <button
              type="button"
              onClick={() => setIsGooglePrePage(false)}
        ...
        {activeTab === 'user' && (
          <div id="auth-tab-user" className="auth-tab-content">
            {/* Clean 1-Click Google OAuth Sign-In Button */}
