    ...
    const handleGoogleResponse = async (response) => {
      try {
        ...
        if (data.success && onGoogleLogin) {
          ...
          onGoogleLogin({ name: 'Google Member', email: 'user@google.com' }, 'token_google_demo');
        }
      } catch (err) {
        console.error("Google auth callback error:", err);
        if (onGoogleLogin) {
          onGoogleLogin({ name: 'Google Member Customer', email: 'customer@google.com' }, 'token_google_demo');
        }
      }
    };
    ...
    script.onload = () => {
      if (window.google?.accounts?.id) {
        try {
          ...
          const btnElem = document.getElementById('google-signin-btn-box');
          if (btnElem) {
      ...
      <div className="modal-card glass auth-card-wide">
        ...
        {activeTab === 'user' && (
          <div id="auth-tab-user" className="auth-tab-content">
            {/* 1-Click Google OAuth Sign-In Section */}
            <div style={{ marginBottom: '20px', padding: '16px', background: 'rgba(30, 58, 95, 0.4)', borderRadius: '16px', border: '1.5px solid #3B82F6', textAlign: 'center' }}>
              <div style={{ fontSize: '13px', fontWeight: 900, color: '#60A5FA', marginBottom: '12px', letterSpacing: '0.3px', textTransform: 'uppercase' }}>
                ⚡ 1-Click Instant Google Sign-In
              </div>

              <div id="google-signin-btn-box" style={{ display: 'flex', justifyContent: 'center', marginBottom: '12px' }}></div>
